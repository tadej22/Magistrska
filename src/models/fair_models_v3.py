...
class FairClassifierV3(BaseEstimator, ClassifierMixin):
    """
    ...
    are then multiplied by overall accuracy of each model and summed together. The class with the highest value is theb
    ...
    **kwargs : dict
    ...
    scores : dict
        A dictionary where the keys are {model_sensitive_values} and the values are the overall accuracy scores on
        training data for each trained model.
    clf_type : class
    ...
    def fit(self, X_A, y):
        ...
        for value in sensitive_values:
            ...
            # Store the accuracy score on the training data of the current trained model
            self.scores[f'model_{value}'] = model.score(X, y)
    ...
    def predict(self, X):
        ...
        are then multiplied by overall accuracy of each model and summed together. The class with the highest value will
        be chosen as the predicted class.
        ...
        for sensitive_value in self.models:
            ...
            # Prediction probabilities are then multiplied by overall model accuracy and summed to the final prediction array
            pred_probas += preds_temp * self.scores[f'model_{sensitive_value}']
